...
mek_med_raf_high <- sum(data$MEK == "Medium" & data$RAF == "High")
...
.left-column[
Local probabilities of MEK expression $P(\text{MEK} \mid \text{RAF})$
```{r, echo=FALSE}
...
Local probabilities of PIP2 expression $P(\text{PIP2} \mid \text{PLCG}, \text{RAF})$
```{r, echo=FALSE, fig.height=5, fig.width=10}
bn.fit.dotplot(bn_fit$PIP2, main = NULL, xlab = NULL, ylab=NULL)
```
...
Usually the structure of the BN is not known. We need to learn the edges and fit the parameters $\theta$.
...
```{r, echo=FALSE}
tibble::as.tibble(signalling.data)
```
]
...
- Learn the structure of a BN using:
    ...
    - `bnlearn::pc.stable`
- Compute the equivalence class of the learned DAGs (`bnlear::cpdag`)
- Make a query: $P(\text{MEK} = \text{high} \mid \text{RAF} = \text{low})$ (`bnlearn::cpquery` or `bnlearn::bn.fit.dotplot`)
...
- `bnlearn` and `pcalg` for structure learning
- `dagitty` and `ggdag` for plotting
